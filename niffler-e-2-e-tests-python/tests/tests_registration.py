...
class TestRegistration:
    def test_success_registration(self, app, envs, generator, check_user_in_db):
        ...
        LoggedExpect(app.register_page.success_registration).to_be_visible()
    ...
    def test_registration_existing_user(self, app, envs, generator):
        ...
        LoggedExpect(app.register_page.error_message).to_have_text(f'Username `{envs.default_user_login}` already exists')
    ...
    def test_registration_with_different_passwords(self, app, envs, generator,
        ...
        LoggedExpect(app.register_page.error_password).to_have_text('Passwords should be equal')
    ...
    def test_registration_with_inappropriate_name_length(self, app, envs, generator,
        ...
        LoggedExpect(app.register_page.error_username).to_have_text('Allowed username length should be from 3 to 50 characters')
        assert not check_user_in_db(username)
    ...
    def test_registration_with_inappropriate_password_length(self, app, envs, generator,
        ...
        LoggedExpect(app.register_page.error_password).to_have_text('Allowed password length should be from 3 to 12 characters')
        assert not check_user_in_db(username)
